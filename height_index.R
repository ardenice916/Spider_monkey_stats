...
#check height_index variable
str(scans)
str(scans$height_index)
summary(scans$height_index)


M0_h_index<-glmer(height_index ~ n_encl + location_focal + time_meal + focal_sex + focal_age + month +
                  (1|focal_group/focal_id), na.action=na.omit, data=scans, family = poisson)

M1_h_index<-glmer(height_index ~ n_encl + location_focal + time_meal + focal_sex + focal_age + month +
                    (1|focal_group), 
...
#anova
anova(M0_h_index, M1_h_index)

anova(M0_h_index, M1_h_index, test = "Chi")

#M0 has lowest AIc

E0<-residuals(M0_h_index, type = "pearson")
str(E0)
E0
summary(E0)
...
overdisp_fun <- function(M0_h_index) {
  rdf <- df.residual(M0_h_index)
  rp <- residuals(M0_h_index,type="pearson")
...
overdisp_fun(M0_h_index)

#underdispersion? ratio is 0.25..., significantly different from 1
